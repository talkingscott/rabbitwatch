...
var group_configs_filename =  process.env.hasOwnProperty('GROUP_CONFIGS') ? process.env.GROUP_CONFIGS : path.join(__dirname, 'group_configs.json');
...
fs.readFile(group_configs_filename, function (err, data) {
  if (err) {
    ...
    var port = process.env.hasOwnProperty('PORT') ? parseInt(process.env.PORT, 10) : 3000,
